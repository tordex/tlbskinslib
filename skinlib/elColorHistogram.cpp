...
void elColorHistogram::draw( cairo_t* cr, int left, int top, int sliceID )
{
	...
	if(vals)
	{
		...
		case startFromBottom:
			draw_h_histogram(cr, left, top, vals, count, max_val);
			break;
		case startFromLeft:
		case startFromRight:
			draw_v_histogram(cr, left, top, vals, count, max_val);
			break;
		}
...
void TxSkin::elColorHistogram::draw_h_histogram(cairo_t* cr, int left, int top, int* data, int count, int max_val)
{
	int x		= left + X();
	int y		= top + Y();
	...
	for(int idx = 0, xx = startX; ; xx += step, idx++)
	{
		...
		{
			yy = y + height - (double) data[idx] * (double) height / (double) max_val;
		} else
		{
			yy = y + (double) data[idx] * (double) height / (double) max_val;
		}
...
void TxSkin::elColorHistogram::draw_v_histogram( cairo_t* cr, int left, int top, int* data, int count, int max_val )
{
	int x		= left + X();
	int y		= top + Y();
	int width	= skin_element::width();
	...
	for(int idx = 0, yy = startY; ; yy += step, idx++)
	{
		...
		{
			xx = x + width - (double) data[idx] * (double) width / (double) max_val;
		} else
		{
			xx = x + (double) data[idx] * (double) width / (double) max_val;
		}
