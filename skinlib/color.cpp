...
TxSkin::def_color TxSkin::g_def_colors[] = 
{
...
TxSkin::color TxSkin::color::from_string( const wchar_t* str )
{
	if(!str)
	{
		return color(0, 0, 0);
	...
	if(str[0] == L'#')
	{
		...
		wchar_t* sss = 0;
		color clr;
		clr.clr.rgbRed		= (byte) wcstol(red.c_str(),	&sss, 16);
		clr.clr.rgbGreen	= (byte) wcstol(green.c_str(),	&sss, 16);
		clr.clr.rgbBlue		= (byte) wcstol(blue.c_str(),	&sss, 16);
		clr.clr.rgbReserved = 255;
		return clr;
	} else if(!wcsncmp(str, L"rgb", 3))
	{
		...
		if(tokens.size() >= 1)	clr.clr.rgbRed		= (byte) _wtoi(tokens[0].c_str());
		if(tokens.size() >= 2)	clr.clr.rgbGreen	= (byte) _wtoi(tokens[1].c_str());
		if(tokens.size() >= 3)	clr.clr.rgbBlue		= (byte) _wtoi(tokens[2].c_str());
		if(tokens.size() >= 4)	clr.clr.rgbReserved	= (byte) _wtoi(tokens[3].c_str());
...
const wchar_t* TxSkin::color::resolve_name( const wchar_t* name )
{
...
TxSkin::color TxSkin::color::parseColor( LPCWSTR clr )
{
	BYTE red	= 0;
	BYTE green	= 0;
	BYTE blue	= 0;
	...
	if(str[0])
	{
		alpha = (BYTE) wcstoul(str, &out, 16);
	}

	return TxSkin::color(red, green, blue, alpha);
}
